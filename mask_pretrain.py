...
from model.modeling.mask import build_mask_model
...
def train(args, cfg):
    model = build_mask_model(cfg).to('cuda')
    if cfg.SOLVER.SYNC_BATCHNORM:
        model = convert_model(model).to('cuda')
    loss_fn = get_loss_fn(cfg.SOLVER.MASK_LOSS_FN)
    ...
    for iteration, (images, targets) in enumerate(data_loader['train'], 1):
        optimizer.zero_grad()
        mask = model(images['mask_input'].to('cuda'), context=images['context_image'].to('cuda'), cood=images['cood'].to('cuda'))
        loss = loss_fn(mask, targets['mask'].to('cuda'))
        loss = loss.mean()
